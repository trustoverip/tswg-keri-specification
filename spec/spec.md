...
Likewise, each controller may create or choose a set or pool of watchers. The controller chooses how the watchers are hosted. It may use a witness service provided by some other party or may directly host its own watchers in its own infrastructure or some combination of the two. Nonetheless, the pool is under the ultimate control of the AID's controller, which means the controller may change its watcher infrastructure at will. Watchers are not AID specific; instead, they watch the KELs of any or all AIDs that are shared with them. Watchers are not explicitly managed by key events. This is so that the watcher infrastructure used by any controller may be kept confidential and, therefore, unknown to potential attackers. It is up to each controller to manage its watcher infrastructure as it sees fit. Each controller uses its own watcher pool to watch the KEL of the other controller. When an AID has witnesses, the watchers of one controller watch the witnesses of the AID of some other controller.  Each controller may use its own watcher pool to watch its own witness pool in order to detect external attacks on those witnesses.
